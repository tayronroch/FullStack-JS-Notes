...
2.  **Efeitos Colaterais (Side Effects):** Evite chamadas de API, acesso ao DOM ou operações de I/O (leitura de arquivos) diretamente no construtor.
    - _Por que?_ O `new` deve ser síncrono e rápido. Além disso, torna impossível instanciar a classe para testes sem disparar o efeito.
3.  **`async` no Construtor:** Construtores **não podem** ser assíncronos. Eles sempre retornam a instância, não uma Promise.
    - _Solução:_ Use um método estático `static create()` ou um método `init()` se precisar de inicialização assíncrona.
...
class ProcessadorPagamento {
  ...
  processar() {
    throw new Error(
      "O método 'processar' deve ser implementado pela classe filha."
    );
  }
...
class PagamentoCartao extends ProcessadorPagamento {
  ...
  processar() {
    ...
    console.log(
      `Cobrando R$ ${this.valor} no cartão final ${ultimosDigitos}... Sucesso!`
    );
  }
}
...
class PagamentoPix extends ProcessadorPagamento {
  ...
  gerarRecibo() {
    return `Recibo PIX: R$ ${this.valor.toFixed(
      2
...
### e. Criando Objetos e Protótipos Manualmente

Antes das classes, e ainda hoje em alguns padrões, usamos `Object.create()` para estabelecer essa ligação de protótipo manualmente. Isso é o JavaScript "puro".
...
// Cria um objeto vazio, mas cujo protótipo é 'prototipoAnimal'
const meuCachorro = Object.create(prototipoAnimal);

meuCachorro.falar(); // "O animal fez um som." (Encontrou no protótipo!)

console.log(meuCachorro.__proto__ === prototipoAnimal); // true
```

---

## 9. Como utilizar Classes para lidar com Exceções

Um dos usos mais poderosos da herança em JavaScript é criar **Erros Personalizados**. Embora a classe nativa `Error` seja útil, muitas vezes precisamos de erros mais específicos para nossa lógica de negócio.

### a. Estendendo a classe `Error`

Ao criar uma classe que estende `Error`, você ganha automaticamente o comportamento padrão de erros (stack trace, mensagem), mas pode adicionar suas próprias propriedades.

```javascript
class AppError extends Error {
  constructor(mensagem, statusCode) {
    super(mensagem); // Chama o construtor de Error para definir a mensagem
    this.name = "AppError"; // Define o nome do erro
    this.statusCode = statusCode; // Propriedade personalizada
  }
}
...
try {
  ...
  if (erro instanceof AppError) {
    console.error(`Erro da Aplicação (${erro.statusCode}): ${erro.message}`);
  } else {
...
### b. Exemplo Prático: Validação e Banco de Dados

Imagine um cenário onde você quer distinguir entre um erro de validação (culpa do usuário) e um erro de banco de dados (culpa do sistema).

```javascript
class ValidationError extends Error {
  constructor(campo) {
    super(`O campo '${campo}' é inválido.`);
...
class DatabaseError extends Error {
  constructor() {
    super("Falha ao conectar ao banco de dados.");
...
try {
  cadastrarUsuario({ email: "emailinvalido" });
} catch (e) {
  if (e instanceof ValidationError) {
    console.log("Por favor, verifique seus dados."); // Feedback amigável
  } else if (e instanceof DatabaseError) {
    console.log("Ops, estamos com problemas técnicos."); // Feedback genérico
    // reportarParaDevOps(e); // Ação interna
  }
}
```

### c. Técnicas Avançadas: Stack Trace e Error Wrapping

Para levar o tratamento de erros ao nível sênior, precisamos falar sobre rastreabilidade.

**1. Limpando o Stack Trace (`Error.captureStackTrace`)**
Em ambientes V8 (Node.js, Chrome), quando você cria um erro personalizado, a própria linha do construtor aparece no log de erro, o que é inútil. Você quer saber onde o erro *foi lançado*, não onde a classe foi definida.
...
class NotFoundError extends Error {
  constructor(resource) {
    ...
    // Remove o construtor do NotFoundError da pilha de erros.
    // O erro vai apontar para a linha que chamou 'new NotFoundError()'.
...
**2. Error Wrapping (Envelopamento de Erros)**
Às vezes, capturamos um erro técnico (banco de dados) e queremos lançar um erro de negócio, mas sem perder a causa original. O JS moderno suporta a propriedade `cause`.

```javascript
class PaymentError extends Error {
  constructor(message, originalError) {
    super(message, { cause: originalError }); // Passa a causa original
...
try {
  ...
  throw new Error("Timeout no gateway de pagamento"); // Erro original
} catch (err) {
  // Lança um novo erro de alto nível, mas anexa o erro técnico
  throw new PaymentError("Não foi possível processar sua compra.", err);
}
